...
export default function StoreCashManagement({ onBack, onViewHistory }: StoreCashManagementProps) {
  ...
  const fetchStoreInfo = useCallback(async () => {
    try {
      ...
      // 캐시 잔액이 충분하고 is_open이 true일 때만 활성화
      setIsActive(data.cash_balance >= 10000 && data.is_open);
    } catch (error) {
  ...
  const updateStoreStatus = async (newValue: boolean) => {
    try {
      ...
      alert(newValue ? '영업이 시작되었습니다.' : '영업이 종료되었습니다.');
    } catch (error) {
      console.error('영업 상태 변경 오류:', error);
      alert('영업 상태 변경에 실패했습니다.');
    }
  };

  const toggleActive = async (newValue: boolean) => {
    // 캐시가 부족한데 활성화하려고 하면 경고
    if (newValue && cashBalance < 10000) {
      alert('캐시 잔액이 1만원 이상이어야 영업을 활성화할 수 있습니다.');
      return;
    }
    ...
    await updateStoreStatus(newValue);
  };
