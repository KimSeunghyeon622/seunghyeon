...
export default function MyReservations({ onBack, onWriteReview }: {
  onBack: () => void;
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.store}>{item.store?.name || '업체명 없음'}</Text>
            <Text style={styles.product}>
              {item.product?.name || '상품명 없음'} x {item.quantity}
            </Text>
            <Text style={styles.amount}>
              {item.total_amount?.toLocaleString() || '0'}원
            </Text>
            <Text style={styles.status}>
              {item.status === 'confirmed' ? '✅ 예약완료' :
               item.status === 'pending' ? '⏳ 대기중' :
               item.status === 'completed' ? '✔️ 완료' :
