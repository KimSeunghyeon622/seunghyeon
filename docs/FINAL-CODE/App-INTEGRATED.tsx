...
type ConsumerScreen = 'login' | 'signup' | 'home' | 'stores' | 'detail' | 'reserve' | 'myreservations' | 'review';
...
export default function App() {
  ...
  // 소비자 상태
  const [consumerScreen, setConsumerScreen] = useState<ConsumerScreen>('login');
  ...
  if (!session) {
    return (
      <View style={{ flex: 1 }}>
        {consumerScreen === 'login' && (
          <LoginScreen onSignup={() => setConsumerScreen('signup')} />
        )}
        {consumerScreen === 'signup' && (
          <SignupScreen onBack={() => setConsumerScreen('login')} />
        )}
      </View>
    );
  }
